...
description: This skill should be used when the user asks to "create an agent", "add an agent", "write a subagent", "agent frontmatter", "when to use description", "agent examples", "agent tools", "autonomous agent", or needs guidance on agent structure, system prompts, triggering conditions, or agent development best practices for DeepSeek Harness plugins.
---

# Agent Development for DeepSeek Harness Plugins
...
- Optional model routing and tool restriction
...
- exact `provider` + `model` pair - Route the child to a specific model
...
Use this prompt pattern (extracted from DeepSeek Harness):
...
3. Leave model unset (inherits the parent route)
4. Define tools (if restricting access)
5. Write system prompt with structure above
6. Save as `com.deepseek.harness/agents/agent-name.md`
...
All `.md` files in `com.deepseek.harness/agents/` are auto-discovered.
...
| model | No | inherit, or exact `provider` + `model` pair | inherit |
...
- **`references/agent-creation-system-prompt.md`** - The exact prompt from DeepSeek Harness
...
3. Create `com.deepseek.harness/agents/agent-name.md` file
