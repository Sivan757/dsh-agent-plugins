...
Complete guide to using MCP tools effectively in DeepSeek Harness plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in DeepSeek Harness tools.
...
2. **Install plugin locally** from a local market source
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `dsh --debug`
