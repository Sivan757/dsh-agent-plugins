...
```bash
cd my-plugin
./validate-hook-schema.sh com.deepseek.harness/hooks/hooks.json
```

## test-hook.sh

Tests individual hook scripts with sample input before deploying to DeepSeek Harness.
...
**Features:**
- Sets up proper environment variables (DSH_PROJECT_DIR, PLUGIN_ROOT)
...
5. **Add to hooks.json**
   ```bash
   # Edit my-plugin/com.deepseek.harness/hooks/hooks.json
   ```

6. **Validate configuration**
   ```bash
   ./validate-hook-schema.sh my-plugin/com.deepseek.harness/hooks/hooks.json
   ```

7. **Test in DeepSeek Harness**
   ```bash
   dsh --debug
   ```
...
- Path in hooks.json is correct (use `${PLUGIN_ROOT}`)
