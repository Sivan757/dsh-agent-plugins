...
This guide shows how to migrate between hook layouts and keep command hooks maintainable and flexible.
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${PLUGIN_ROOT}/scripts/analyze-bash-input.sh",
          "timeout": 15
        }
      ]
    }
  ...
  "PreToolUse": [
    {
      "matcher": "Write|Edit",
      "hooks": [
        {"type": "command", "command": "bash ${PLUGIN_ROOT}/scripts/check.sh"}
      ]
    }
  ]
}
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        ...
        {
          "type": "command",
          "command": "bash ${PLUGIN_ROOT}/scripts/deep-check.sh",
          "timeout": 15
        }
      ]
    }
...
- [ ] Set appropriate timeout (usually 5-30s per hook)
...
my-plugin/
├── plugin.json
├── com.deepseek.harness/hooks/hooks.json
└── scripts/
    ├── validate-bash.sh
    ├── validate-write.sh
    └── check-tests.sh
...
my-plugin/
├── plugin.json
├── com.deepseek.harness/hooks/hooks.json      # Now uses command hooks
...
Well-structured command hooks keep plugins maintainable and powerful. Reserve each hook for deterministic checks and external tool integration.
