...
This reference provides common, proven patterns for implementing DeepSeek Harness hooks. Use these patterns as starting points for typical hook use cases.

## Pattern 1: Security Validation

Block dangerous file writes using command hooks:
...
FLAG_FILE="$DSH_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart DeepSeek Harness after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$DSH_PROJECT_DIR/.agents/my-plugin.local.json"
