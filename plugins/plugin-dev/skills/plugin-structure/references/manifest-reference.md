...
The manifest MUST be at the plugin root. DeepSeek Harness will not recognize plugins without this file in the correct location.
...
- Plugin identification in DeepSeek Harness
...
**Default**: `"./com.deepseek.harness/hooks/hooks.json"`
...
When DeepSeek Harness loads components:
...
DeepSeek Harness validates the manifest on plugin load:
...
```json
{
  "hooks": "com.deepseek.harness/hooks/hooks.json"  // ❌ No ./
}
```
Fix: Add ./ prefix
```json
