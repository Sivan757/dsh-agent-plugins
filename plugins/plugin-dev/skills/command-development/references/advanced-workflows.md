...
run `gh pr view $1 --json title,body,author,files`

## Step 2: Review Files
Files changed: run `gh pr diff $1 --name-only`
...
## Step 3: Run Checks
Test status: run `gh pr checks $1`
...
Latest commit: run `git log -1 --format=%H`
...
Previous version: run `current-version.sh`
