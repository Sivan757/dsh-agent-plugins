...
Comprehensive guide to creating commands that gather user feedback and make decisions through the ask_user_question tool.
...
For these cases, use the **ask_user_question tool** within command execution rather than relying on command arguments.
...
allowed-tools: ask_user_question, Write
---
...
Based on the answers received from ask_user_question:
...
allowed-tools: ask_user_question, Read, Write, Bash
---
...
Use ask_user_question to ask about deployment basics.
...
Use ask_user_question to ask about:
...
Use ask_user_question for final confirmation:
- header: "Confirm"
...
```markdown
Use ask_user_question for enabling features:
...
## Command Patterns with ask_user_question
...
allowed-tools: ask_user_question, Bash
---
...
Use ask_user_question with multiple questions in one call:
...
allowed-tools: ask_user_question, Read, Write
---
...
Use ask_user_question for member details:
...
```markdown
# Handle ask_user_question Responses

After calling ask_user_question, verify answers received:
...
Use ask_user_question to collect settings.
...
allowed-tools: ask_user_question, Write, Read
---
...
Use ask_user_question for core settings.
...
Use ask_user_question for settings based on Phase 1 answers.
...
allowed-tools: ask_user_question, Bash, Read
---
...
- Current language: run `detect-language.sh`
- Existing frameworks: run `detect-frameworks.sh`
- Available tools: run `check-tools.sh`
...
Use ask_user_question for options that require explanation.
```
...
Now use ask_user_question to choose:
...
- Verify ask_user_question in allowed-tools
...
With ask_user_question, commands become interactive wizards that guide users through complex decisions while maintaining the clarity that simple arguments provide for straightforward inputs.
