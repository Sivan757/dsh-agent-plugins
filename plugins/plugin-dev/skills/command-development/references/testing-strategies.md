...
if echo "$FRONTMATTER" | grep -q "^model:"; then
  ...
  if ! echo "inherit inherit inherit" | grep -qw "$MODEL"; then
    echo "ERROR: Invalid model '$MODEL' (must be exact model ids)"
    exit 1
  fi
...
**What to test:**
- Command appears in the slash menu
...
Test output: run `echo "Hello from bash"`

Analysis of output above...
EOF

# Test in DeepSeek Harness
> /test-bash
# Verify:
...
```bash
# Monitor DeepSeek Harness during command execution
# In terminal 1:
dsh --debug

# In terminal 2:
watch -n 1 'ps aux | grep dsh'
...
- [ ] Description is clear in the command list
...
**Issue: Command not appearing in the slash menu**
