...
Comprehensive guidance on creating DeepSeek Harness slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Inline bash execution in commands (source-format feature; verify harness support)
...
- Inline bash execution (source-format feature)
...
DeepSeek Harness activates this skill when users:
...
- Files: @path/to/file
- Inline bash: run the command first, then include its output
```
...
**Bash execution:**
- Inline `command` — run it and splice the output into the prompt (source-format feature; the portable pattern instructs the agent to run it)

## Frontmatter Fields Quick Reference

| Field | Purpose | Example |
|-------|---------|---------|
| `description` | Brief description for the command list | `"Review code for issues"` |
| `allowed-tools` | Restrict tool access | `Read, Bash(git:*)` |
| `model` | Specify model | exact model ids or inherit |
...
2. **Clear descriptions**: Make discoverable in the command list
...
6. Test examples work with current DeepSeek Harness
