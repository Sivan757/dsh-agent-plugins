...
3. **Select Configuration**:
   - **Model**: Omit the field, or use `inherit`, unless the user pins an exact `provider` + `model` pair
   - **Tools**: Recommend minimal set needed, or omit for full access

4. **Generate Agent File**: Use the write tool to create `com.deepseek.harness/agents/[identifier].md`:
   ```markdown
   ---
   name: [identifier]
...
- **Model:** [choice or "inherits"]
- **Tools:** [list or "all tools"]

### File Created
`com.deepseek.harness/agents/[identifier].md` ([word count] words)
...
Validate with: `scripts/validate-agent.sh com.deepseek.harness/agents/[identifier].md`
...
This agent automates agent creation using the proven patterns from DeepSeek Harness's internal implementation, making it easy for users to create high-quality autonomous agents.
