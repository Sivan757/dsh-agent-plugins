...
Guide the user through creating a complete, high-quality DeepSeek Harness plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
- **Use todo_write**: Track all progress throughout all phases
...
**MUST load plugin-structure skill** using the `skill` tool before this phase.
...
2. Analyze plugin requirements and determine needed components:
   ...
   - **Commands**: User-invoked slash entries with argument hints (deploy, configure, analyze)
   - **Agents**: Autonomous tasks? (validation, generation, analysis)
   - **Hooks**: Event-driven automation? (validation, notifications)
   - **MCP**: External service integration? (databases, APIs)
   - **Settings**: User configuration? (.local.md files)

   > **Note:** Commands live in `com.deepseek.harness/commands/*.md`; user-invocable skills sit in `skills/<name>/SKILL.md`. Choose by shape — an argument-hint prompt shortcut is a command, knowledge plus a procedure is a skill.
...
- Skills: Load skill-creator skill
...
1. Load skill-creator skill using the `skill` tool
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description` and `argument-hint` frontmatter; write instructions for the agent (not TO user)
...
1. Load command-development skill using the `skill` tool
2. For each command:
...
1. Load agent-development skill using the `skill` tool
...
1. Load hook-development skill using the `skill` tool
2. For each hook:
   - Create com.deepseek.harness/hooks/hooks.json with hook configuration
...
1. Load mcp-integration skill using the `skill` tool
...
1. Load plugin-settings skill using the `skill` tool
...
5. **Test hook configuration** (if plugin has hooks):
   - Run validate-hook-schema.sh on com.deepseek.harness/hooks/hooks.json
...
**Goal**: Test that plugin works correctly in DeepSeek Harness

**Actions**:

1. **Installation instructions**:
   - Show user how to test locally: add the plugin directory as a local source on the market page, refresh, and install
   - Or point the market source at the plugin directory while iterating
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `dsh --debug` to see hook execution
...
- **Use todo_write** to track progress at every phase
- **Load skills with the `skill` tool** when working on specific component types
...
- **Phase 5**: skill-creator, agent-development, hook-development, mcp-integration, plugin-settings, command-development (as needed)
