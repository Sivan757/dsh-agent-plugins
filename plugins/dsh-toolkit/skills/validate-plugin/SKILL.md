...
description: Validate a plugin directory against the Agent Plugins v1 specification and DeepSeek Harness layout conventions. Use when the user asks to "validate my plugin", "check my plugin", "verify plugin.json", or reports a plugin that fails to appear in the market.
---

# Validate Plugin

Check one plugin directory and report every problem with its fix. Read-only: never modify the plugin under validation.
...
For a v1 plugin, verify against the specification:
...
5. Every component path (namespace commands, agents, hooks, LSP declarations) stays inside the plugin root; flag any `..` segment or symlink escape.
...
5. `com.deepseek.harness/hooks/hooks.json`: top-level `hooks` object keyed by event names (`PreToolUse`, `PostToolUse`, `SessionStart`, `UserPromptSubmit`, `Stop`, `SubagentStart`, `SubagentStop`); each group carries a `hooks` array whose entries have a non-empty `command`, and a `matcher` compiles as a regular expression.
