...
argument-hint: [--local]
---
Initialize the project instruction documents for dsh. Follow the init-project skill's phases exactly:
...
Apply every rule from the init-project skill: instruction lines only where removal would cause an agent mistake, commands recorded verbatim, deeper detail referenced rather than copied.
