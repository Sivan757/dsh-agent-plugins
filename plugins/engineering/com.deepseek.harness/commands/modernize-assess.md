...
Read `${PLUGIN_ROOT}/workflows/portfolio-assess.meta.json` and
`${PLUGIN_ROOT}/workflows/portfolio-assess.js`, then pass the meta file's
contents as `meta` and the script file's contents (the JavaScript body, with
no `export const meta` line) as `script`:

```
workflow({
  meta: <contents of ${PLUGIN_ROOT}/workflows/portfolio-assess.meta.json>,
  script: <contents of ${PLUGIN_ROOT}/workflows/portfolio-assess.js>,
