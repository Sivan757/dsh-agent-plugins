...
Read `${PLUGIN_ROOT}/workflows/harden-scan.meta.json` and
`${PLUGIN_ROOT}/workflows/harden-scan.js`, then pass the meta file's contents
as `meta` and the script file's contents (the JavaScript body, with no
`export const meta` line) as `script`:

```
workflow({
  meta: <contents of ${PLUGIN_ROOT}/workflows/harden-scan.meta.json>,
  script: <contents of ${PLUGIN_ROOT}/workflows/harden-scan.js>,
  args: { system: "$1" }
})
```
...
**Fallback — direct subagent** (older DeepSeek Harness builds without the
