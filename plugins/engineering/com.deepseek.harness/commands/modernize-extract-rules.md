...
Read `${PLUGIN_ROOT}/workflows/extract-rules.meta.json` and
`${PLUGIN_ROOT}/workflows/extract-rules.js`, then pass the meta file's
contents as `meta` and the script file's contents (the JavaScript body, with
no `export const meta` line) as `script`:

```
workflow({
  meta: <contents of ${PLUGIN_ROOT}/workflows/extract-rules.meta.json>,
  script: <contents of ${PLUGIN_ROOT}/workflows/extract-rules.js>,
...
DeepSeek Harness build), use Method B.
