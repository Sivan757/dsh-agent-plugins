...
In a pull-request context, read the PR's diff too.
...
| CI workflow | the runner | dispatch it, read the run |
...
- **Project skills that verify or run things** — if one matches your
  surface (a CLI verifier for a CLI change, a run-skill that builds
  and launches), route to it. It knows readiness signals and env
  gotchas you don't. Mismatched surface → skip that one, try the
  next. Stale verifier (fails on mechanics unrelated to the change) →
  ask the user whether to patch it; don't FAIL the change for
  verifier rot.
- **Neither** — cold start from README/package.json/Makefile. Timebox
  it; stuck → BLOCKED with exactly where you stopped.
...
- Changed an internal function? Find the CLI command / request /
  render that reaches it. Run that.
...
- **BLOCKED** — couldn't reach a state where the change is observable,
  ...
  blocker → say exactly where you stopped. No surface → one line why.
