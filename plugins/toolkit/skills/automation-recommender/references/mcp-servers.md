...
**Connection methods:**
1. **Project config** (`.agents/mcp.json`) - Available only in that directory
2. **User config** (`~/.agents/mcp.json`) - Available across all projects
3. **Checked-in `.agents/mcp.json`** - Available to entire team (recommended!)

**Tip**: Check `.agents/mcp.json` into git so your whole team gets the same MCP servers.
...
| LLM provider SDK | context7 for provider docs |
