...
Plugins are installable collections of skills, commands, agents, and hooks. Install them from the agent plugins market page: add a git source for the repository, then install the suite there.
...
|--------|----------|--------------|
| **plugin-dev** | Building DeepSeek Harness plugins | Skills for creating skills, hooks, commands, agents |
...
Install suites on the agent plugins market page:

1. Add the repository as a git source.
2. Refresh the source, open the suite, and install it.

The market page lists installed suites, and each one can be disabled or removed there.
...
- User wants to install DeepSeek Harness automations from a shared marketplace source
- User needs multiple related capabilities
- Team wants standardized workflows
- First-time DeepSeek Harness setup