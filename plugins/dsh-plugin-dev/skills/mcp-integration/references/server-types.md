...
Complete reference for all MCP server types supported in DeepSeek Harness plugins.
...
1. **Startup**: DeepSeek Harness spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire DeepSeek Harness session
4. **Shutdown**: Process terminated when DeepSeek Harness exits
...
DeepSeek Harness handles OAuth flow:
