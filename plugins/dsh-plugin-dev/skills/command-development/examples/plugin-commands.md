...
Practical examples of commands designed for DeepSeek Harness plugins, demonstrating plugin-specific patterns and features.
...
run `node ${PLUGIN_ROOT}/scripts/quality-check.js $1`
...
**Security scan:**
run `bash ${PLUGIN_ROOT}/scripts/security-scan.sh $1`

**Performance analysis:**
run `bash ${PLUGIN_ROOT}/scripts/perf-analyze.sh $1`

**Best practices check:**
run `bash ${PLUGIN_ROOT}/scripts/best-practices.sh $1`
...
run `bash ${PLUGIN_ROOT}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
run `bash ${PLUGIN_ROOT}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
run `bash ${PLUGIN_ROOT}/scripts/run-tests.sh`

**Step 4 - Package release:**
run `bash ${PLUGIN_ROOT}/scripts/package.sh $1`
...
Current git state: run `git rev-parse --short HEAD`

Build info: run `cat package.json | grep -E '(name|version)'`
...
Note: This uses the subagent_run tool to launch the plugin's code-reviewer agent for thorough analysis.
```
...
Run plugin analyzer: run `node ${PLUGIN_ROOT}/scripts/analyze.js $1`
...
Validate environment argument: run `echo "$1" | grep -E "^(dev|staging|prod)$" && echo "VALID" || echo "INVALID"`

Check build script exists: run `test -x ${PLUGIN_ROOT}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: run `test -f ${PLUGIN_ROOT}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${PLUGIN_ROOT}/config/$1.json

**Execute build:** run `bash ${PLUGIN_ROOT}/scripts/build.sh $1 2>&1`

**Validation results:** run `bash ${PLUGIN_ROOT}/scripts/validate-build.sh $1 2>&1`
...
Determine check level: run `echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: run `bash ${PLUGIN_ROOT}/scripts/test-full.sh`
- Security scan: run `bash ${PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: run `bash ${PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: run `bash ${PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: run `bash ${PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: run `bash ${PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
run `node ${PLUGIN_ROOT}/scripts/script-name.js $1`
```
...
```markdown
Validate input: run `echo "$1" | grep -E "^pattern$" && echo "OK" || echo "ERROR"`
```
Use for: Validating command arguments

### Pattern: Resource Validation
```markdown
Check exists: run `test -f ${PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   dsh /command-name args
   ```

2. **Verify ${PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   run `echo "Plugin root: ${PLUGIN_ROOT}"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && dsh /command-name
   cd /other/project && dsh /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   run `ls -la ${PLUGIN_ROOT}/scripts/`
   run `ls -la ${PLUGIN_ROOT}/config/`
   ```
...
1. **Using relative paths instead of ${PLUGIN_ROOT}:**
   ...
   # Correct
   run `node ${PLUGIN_ROOT}/scripts/analyze.js`
   ```

2. **Forgetting to allow required tools:**
   ```markdown
   # Missing allowed-tools
   run `bash script.sh`  # Will fail without Bash permission

   # Correct
   ---
   allowed-tools: Bash(*)
   ---
   run `bash ${PLUGIN_ROOT}/scripts/script.sh`
   ```
...
3. **Not validating inputs:**
   ...
   Validate: run `echo "$1" | grep -E "^(dev|staging|prod)$" || echo "INVALID"`
