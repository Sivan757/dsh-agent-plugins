...
- Git repository: run `git rev-parse --git-dir 2>/dev/null`
...
Target: $1
Current version: run `cat version.txt`
New version: $2
...
if [ $? -ne 0 ]; then
  ...
  NEED HELP?
  - Check the slash menu for related commands
...
Install with:
\`\`\`
Install plugin-name from the market page
\`\`\`
