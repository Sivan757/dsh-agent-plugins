...
4. **Register components**: Make available to DeepSeek Harness
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during DeepSeek Harness initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → DeepSeek Harness looks up → Executes
**Agents**: Task arrives → DeepSeek Harness evaluates capabilities → Selects agent
**Skills**: Task context matches description → DeepSeek Harness loads skill
**Hooks**: Event occurs → DeepSeek Harness calls matching hooks
...
**Note**: DeepSeek Harness doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, DeepSeek Harness doesn't support file references.
